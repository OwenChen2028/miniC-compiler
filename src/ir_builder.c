...
LLVMBasicBlockRef genIRStmt(astStmt *stmt, LLVMBasicBlockRef startBB) {
  if (!stmt)
    return NULL;
  ...
  case ast_call: { // must be print call
    ...
    LLVMTypeRef type = LLVMFunctionType(LLVMVoidType(), params, 1, 0);
    LLVMBuildCall2(builder, type, print_func, args, 1, "");
    return startBB;
  }
  ...
  case ast_while: {
    ...
    LLVMValueRef condition = genIRExpr(stmt->whilen.cond);
    ...
    LLVMBasicBlockRef trueExitBB = genIRStmt(&stmt->whilen.body->stmt, trueBB);
  ...
  case ast_if: {
    LLVMPositionBuilderAtEnd(builder, startBB);
    LLVMValueRef condition = genIRExpr(stmt->ifn.cond);
  ...
  case ast_ret: {
    LLVMPositionBuilderAtEnd(builder, startBB);
    LLVMValueRef ret_val = genIRExpr(stmt->ret.expr);
...
void rmUnreachableBB() {
  ...
  for (LLVMBasicBlockRef basicBlock = entryBB; basicBlock;) {
...
void build_ir(astNode *root) {
  ...
  LLVMBasicBlockRef exitBB = genIRStmt(&node->func.body->stmt, entryBB);

  LLVMValueRef terminator = LLVMGetBasicBlockTerminator(exitBB);
  if (!terminator) {
    LLVMPositionBuilderAtEnd(builder, exitBB);
    LLVMBuildStore(builder, LLVMConstInt(LLVMInt32Type(), 0, 0), ret_ref);
    LLVMBuildBr(builder, retBB);
  }
