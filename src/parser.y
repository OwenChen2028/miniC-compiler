...
% union {
  ...
  std::vector<astNode *> *nvec;
    ...
    % token<ival> NUM % token<sval> NAME % token INT VOID %
    token IF ELSE WHILE % token RETURN % token EQ NE GE LE % token EXTERN %
    token READ PRINT

    % start program

    % nonassoc TAG % nonassoc ELSE

    % type<node> printDec readDec funcDec %
    type<node> block varDec stmt varAssign % type<node> readCall printStmt %
    type<node> ifStmt whileStmt retStmt %
    type<node> relExpr intExpr term posTerm % type<nvec> blockBody decls stmts

    % % program : printDec readDec funcDec {
